...
Locator: locator('h3[class=\'Create Account\']')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h3[class=\'Create Account\']')
...
```ts
  ...
  12 | 
  13 |     //Verify if opening balance is 50000
> 14 |     await expect(page.locator("h3[class='Create Account']")).toBeVisible();
     |                                                              ^ Error: expect(locator).toBeVisible() failed
  15 |     await expect(page.locator("h3[class='Create Account']")).toHaveText('50,000.00');
  16 | 
  17 | 
